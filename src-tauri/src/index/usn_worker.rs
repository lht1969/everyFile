    ...
    let volume = match Volume::from_drive_letter(drive_letter) {
        ...
        Err(e) => {
            let _ = resp_tx.send(UsnResponse::Error {
                drive_letter, message: format!("Failed to open volume {} for journal: {}", drive_letter, e),
            });
            return;
        }
    };
    ...
    let (_journal_id, last_usn) = match journal.query(true) {
        ...
        Err(e) => {
            ...
            match journal.create_or_update(journal_max_size, allocation_delta) {
                ...
                Err(e2) => {
                    let _ = resp_tx.send(UsnResponse::Error {
                        drive_letter, message: format!("Journal create failed for {}: {}", drive_letter, e2),
                    });
                    return;
                }
            }
...
fn handle_full_scan_legacy(
    ...
    let volume = match Volume::from_drive_letter(drive_letter) {
        ...
        Err(e) => {
            let _ = resp_tx.send(UsnResponse::Error {
                drive_letter, message: format!("Failed to open volume {}: {}", drive_letter, e),
            });
            return;
        }
    };
...
fn handle_poll_changes(
    ...
    let volume = match volumes.get(&drive_letter) {
        ...
        None => {
            let _ = resp_tx.send(UsnResponse::Error {
                drive_letter, message: format!("No volume handle for drive {}", drive_letter),
            });
            return;
        }
    };
    ...
    let last_usn = match last_usn_map.get(&drive_letter) {
        ...
        None => {
            let _ = resp_tx.send(UsnResponse::Error {
                drive_letter, message: format!("No last USN recorded for drive {}", drive_letter),
            });
            return;
        }
    };
    ...
    let journal_data = match journal.query(false) {
        ...
        Err(e) => {
            log::warn!("[USN] Failed to query journal for {}: {}", drive_letter, e);
            let _ = resp_tx.send(UsnResponse::Error {
                drive_letter, message: format!("Failed to query journal for {}: {}", drive_letter, e),
            });
            return;
        }
    };
    ...
    let vol_handle = match ntfs_mft::open_volume_handle(drive_letter) {
        ...
        None => {
            let _ = resp_tx.send(UsnResponse::Error {
                drive_letter, message: format!("Failed to open volume handle for drive {}", drive_letter),
            });
            return;
        }
    };
    ...
    let (records, next_start_usn) =
        match read_usn_records_direct(vol_handle, journal_data.journal_id, start_usn) {
            ...
            Err(e) => {
                ...
                let _ = resp_tx.send(UsnResponse::Error {
                    drive_letter, message: format!("Failed to read journal for {}: {}", drive_letter, e),
                });
                return;
            }
        };
